...
export function TopologyGraph({ selectedId, onSelect, nodes, edges, topologyMode, isPlaying, timelinePhase, simulationFrozen }: Props) {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      const cfg: SimulationConfig = {
        ...
        timelineEnergy: isPlaying ? timelinePhase * 0.7 : timelinePhase * 0.4,
      };
  ...
  return <div className="absolute inset-0 overflow-hidden bg-[#080f11]"><svg viewBox="0 0 1600 900" className="h-full w-full">
    ...
    {nodes.map((node) => { const pos = positions[node.id]; if (!pos) return null; const p = project(pos.x, pos.y); const selected = node.id===selectedId; const hovered = node.id===hoveredId; const focused = !focusSet || focusSet.has(node.id); const dim = focused ? 1 : 0.2; const color = node.type === "anomaly" ? "#ff9387" : "#76e9ff"; return <g key={node.id} className="cursor-pointer transition-opacity" onMouseEnter={()=>setHoveredId(node.id)} onMouseLeave={()=>setHoveredId(null)} onClick={()=>onSelect(node.id)}>
      <circle cx={p.x} cy={p.y} r={node.size * (4 + node.severity * 1.5)} fill={color} fillOpacity={(hovered||selected?0.18:0.07)*dim} />
      <circle cx={p.x} cy={p.y} r={node.size * 1.2} fill={color} fillOpacity={0.92*dim} />
      {selected && <circle cx={p.x} cy={p.y} r={node.size*5} stroke="#ffb4ab" strokeDasharray="4 6" fill="none" />}
    </g>; })}
  </svg>
