...
export function TopologyGraph({ selectedId, onSelect, nodes, edges, topologyMode, isPlaying, timelinePhase, simulationFrozen }: Props) {
  const [hoveredId, setHoveredId] = useState<string | null>(null);
  const [positions, setPositions] = useState<Record<string, { x: number; y: number }>>({});
  const [speed, setSpeed] = useState(0.65);
  const [energyIntensity, setEnergyIntensity] = useState(0.45);
  const [stabilization, setStabilization] = useState(1.08);
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      const cfg: SimulationConfig = {
        ...
        energyIntensity,
        stabilization,
        frozen: simulationFrozen || frozenOverride,
        timelineEnergy: isPlaying ? timelinePhase * 0.45 : timelinePhase * 0.2,
        focusNodeId: hoveredId ?? selectedId ?? null,
      };
  ...
  return <div className="absolute inset-0 overflow-hidden bg-[#04090d]"><div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_36%_30%,rgba(81,132,255,0.12),transparent_44%),radial-gradient(circle_at_68%_52%,rgba(255,164,126,0.1),transparent_36%),linear-gradient(180deg,rgba(3,8,13,0.2),rgba(3,8,13,0.9))]"/><svg viewBox="0 0 1600 900" className="h-full w-full">
    <defs>
      <radialGradient id="energyPulse"><stop offset="0%" stopColor="#ff8f6f" stopOpacity="0.22" /><stop offset="100%" stopColor="#ff8f6f" stopOpacity="0" /></radialGradient>
    </defs>
    ...
    {edges.map((edge) => { const a = positions[edge.source]; const b = positions[edge.target]; if (!a || !b) return null; const p1 = project(a.x, a.y); const p2 = project(b.x, b.y); const mx = (p1.x + p2.x) / 2; const my = (p1.y + p2.y) / 2 - 24; const focused = !!(focusSet && focusSet.has(edge.source) && focusSet.has(edge.target)); return <path key={edge.id} d={`M${p1.x},${p1.y} Q${mx},${my} ${p2.x},${p2.y}`} stroke={focused ? "#91ecff" : "#3f6e84"} strokeWidth={focused ? 1.7 : 1} strokeOpacity={focused ? 0.8 : 0.25} fill="none" />; })}
    {edges.slice(0, 8).map((edge, i) => { const a = positions[edge.source]; const b = positions[edge.target]; if (!a || !b) return null; const p1 = project(a.x, a.y); const p2 = project(b.x, b.y); const t = (timelinePhase * 1.7 + i * 0.09) % 1; const x = p1.x + (p2.x - p1.x) * t; const y = p1.y + (p2.y - p1.y) * t; return <circle key={`flow-${edge.id}`} cx={x} cy={y} r={1.2} fill="#9fdff0" opacity={0.12} />; })}
    {nodes.map((node) => { const pos = positions[node.id]; if (!pos) return null; const p = project(pos.x, pos.y); const selected = node.id===selectedId; const hovered = node.id===hoveredId; const focused = !focusSet || focusSet.has(node.id); const dim = focused ? 1 : 0.2; const color = node.type === "anomaly" ? "#ffb39e" : node.type === "warning" ? "#ffd58a" : "#9bd8f0"; return <g key={node.id} className="cursor-pointer transition-opacity" onMouseEnter={()=>setHoveredId(node.id)} onMouseLeave={()=>setHoveredId(null)} onClick={()=>onSelect(node.id)}>
      <circle cx={p.x} cy={p.y} r={node.size * (4 + node.severity * 1.5)} fill={color} fillOpacity={(hovered||selected?0.16:0.05)*dim} />
      <circle cx={p.x} cy={p.y} r={node.size * 1.2} fill={color} fillOpacity={0.9*dim} />
      {selected && <circle cx={p.x} cy={p.y} r={node.size*5} stroke="#ffd3c4" strokeDasharray="4 6" fill="none" />}
    </g>; })}
  </svg>
  <div className="absolute bottom-4 right-4 z-20 w-44 rounded-lg border border-cyan-300/30 bg-[#0c1214]/60 p-2 text-[10px] text-[#c8d5d9] backdrop-blur">
    <p className="font-semibold text-cyan-100">Simulation</p>
    <label className="mt-1 block">Speed <input type="range" min={0.35} max={1.3} step={0.1} value={speed} onChange={(e)=>setSpeed(Number(e.target.value))} className="w-full"/></label>
    <label className="mt-1 block">Energy <input type="range" min={0.2} max={1.1} step={0.1} value={energyIntensity} onChange={(e)=>setEnergyIntensity(Number(e.target.value))} className="w-full"/></label>
    <label className="mt-1 block">Stability <input type="range" min={0.7} max={1.4} step={0.1} value={stabilization} onChange={(e)=>setStabilization(Number(e.target.value))} className="w-full"/></label>
