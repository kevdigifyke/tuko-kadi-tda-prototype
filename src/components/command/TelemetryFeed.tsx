...
export default function TelemetryFeed() {
  ...
  return (
    <motion.div
      className="relative h-full overflow-hidden p-4 bg-zinc-950 border-l border-zinc-800"
      animate={isEscalated ? { boxShadow: ["inset 0 0 0 rgba(244,63,94,0)", "inset 0 0 24px rgba(244,63,94,0.24)", "inset 0 0 0 rgba(244,63,94,0)"] } : {}}
    ...
    >
      <div className="mb-3 flex items-center justify-between">
        ...
        >
          {liveEventCount} events · L{anomalyLevel}
        </motion.div>
      </div>
      ...
      <AnimatePresence>
        {activeAlerts[0] && (
          <motion.div
            ...
            exit={{ y: -16, opacity: 0 }}
            className="mb-3 rounded-lg border border-rose-400/50 bg-rose-500/15 p-2 text-xs text-rose-100"
          >
            CRITICAL ALERT · {activeAlerts[0].title} — {activeAlerts[0].county}
          </motion.div>
        )}
      </AnimatePresence>

      <div ref={streamRef} className="h-[calc(100%-4rem)] overflow-y-auto pr-1 space-y-2">
        <AnimatePresence initial={false}>
          {events.map((event) => (
            <motion.div
              ...
              className={`rounded-xl border p-3 backdrop-blur-sm ${severityStyles[event.severity]}`}
            >
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2 text-[10px] uppercase tracking-wide">
                <span className="rounded border border-zinc-600 px-2 py-0.5 text-zinc-200">{event.category}</span>
                <span className="rounded border border-zinc-600 px-2 py-0.5 text-zinc-200">{event.county}</span>
                <span className="rounded border border-zinc-600 px-2 py-0.5 text-zinc-300">{event.status}</span>
              </div>
            </motion.div>
          ))}
        </AnimatePresence>
