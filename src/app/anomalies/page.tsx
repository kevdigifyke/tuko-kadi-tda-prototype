...
return <AppShell><div className="space-y-4"><h1 className="text-display">Anomaly Network Command</h1><div className="relative min-h-[calc(100vh-140px)] overflow-hidden rounded-xl border border-white/10 bg-[#080f11] pb-[152px] xl:pb-[168px]"><TdaGraph selectedId={selectedId} onSelect={setSelectedId}/><div className="absolute inset-0 hidden xl:block"><div className="absolute left-6 top-6"><GesturePanel/></div><div className="absolute right-6 top-6 w-[360px] space-y-3"><EvidencePanel cluster={selected}/><WarningStrip text="Cross-race mismatch • Severity elevated"/><WarningStrip text="Late upload spike • Requires review"/></div><div className="absolute bottom-6 left-6 right-6 z-20"><TimelineScrubber isPlaying={isPlaying} onToggle={()=>setIsPlaying(s=>!s)}/></div></div></div><div className="space-y-3 xl:hidden"><GesturePanel/><EvidencePanel cluster={selected}/><WarningStrip text="Cross-race mismatch • Severity elevated"/><WarningStrip text="Late upload spike • Requires review"/><TimelineScrubber isPlaying={isPlaying} onToggle={()=>setIsPlaying(s=>!s)}/></div></div></AppShell>}
